; Fish and sharks are both breeds of turtles
breed [fish a-fish]
breed [sharks shark]
...
to setup
  ...
  ask patches
  [
    set pcolor one-of [86 36] ;86 = cyan-ish, 36 = brown-ish
    if pcolor = 36 [set countdown algae-regrowth-time]
  ]
...
to go
  ...
  if count sharks < 2 and count fish > 5000 [user-message "The fish have inherited the seven seas!" stop ]
...
to grow-algae
  ...
  [
    ifelse countdown <= 0
    [set pcolor 36
     set countdown algae-regrowth-time]
...
to reproduce-fish
  let fishcount count fish-here
  if fishcount >= 2
  [
    if random-float 100 < fish-reproduce-probability
    [
      set energy (energy / 4)
...
to reproduce-sharks
  let sharkcount count sharks-here
  if sharkcount >= 2
  [
    if random-float 100 < sharks-reproduce-probability
    [
      set energy (energy / 2)
